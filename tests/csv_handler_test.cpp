...
TEST_CASE("isValidCSV - Valid CSV file") {
...
TEST_CASE("isValidCSV - Invalid extension") {
...
TEST_CASE("isValidCSV - Non-existent file is rejected early") {
...
TEST_CASE("isValidCSV - Empty file") {
...
TEST_CASE("isValidCSV - Inconsistent columns") {
...
TEST_CASE("isValidCSV - Duplicate lines") {
...
TEST_CASE("isValidCSV - Only one column") {
...
TEST_CASE("isValidCSV - CSV with an empty line is rejected") {
...
TEST_CASE("generate - basic case") {
  const int minRowCount = 2;
  const int colCount = 3;

  std::string result = CSVHandler::generate(minRowCount, colCount,
                                            2); // at least 2 rows, 3 columns
  std::vector<std::vector<std::string>> resultAsVector =
      CSVHandler::stringToVector(result);

  CHECK(resultAsVector.size() >= minRowCount);

  for (auto row : resultAsVector) {
