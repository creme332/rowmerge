...
std::string
CompressedTableAlgorithm::solve(std::vector<std::vector<std::string>> &input) {
  std::vector<std::vector<std::string>> compressedTable = compressTable(input);
...
/**
 * @brief Determines if two rows can be merged based on a single differing
 * value.
 *
 ...
 */
bool CompressedTableAlgorithm::canMerge(const std::vector<std::string> &row1,
 ...
 */
std::vector<std::vector<std::string>> CompressedTableAlgorithm::compressTable(
